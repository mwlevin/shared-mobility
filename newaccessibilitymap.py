...
# File paths
travel_times_file = '/Users/mobjoon/Downloads/scenario1/travel_times.txt'
...
def get_color(value, min_val, max_val):
    ...
    cmap = cm.RdYlGn_r  # Red-Yellow-Green colormap, reversed to make green=low, red=high
    rgba_color = cmap(norm(value))
    return colors.to_hex(rgba_color)

def create_combined_accessibility_map(travel_times_data, node_coords, network_links, selected_origin, store_locations, excluded_nodes):
    # Filter out excluded nodes from travel times
    travel_times = {k: v for k, v in travel_times_data[selected_origin].items() if k not in excluded_nodes}
    ...
    grid_values = griddata(points, values, grid_points, method='cubic', fill_value=-1)
    ...
    # Get min/max times for color scaling
    ...
    for i in range(len(grid_lat)-1):
        for j in range(len(grid_lon)-1):
            if grid_values[i,j] > 0:
                ...
                color = get_color(grid_values[i,j], min_time, max_time)
                folium.Polygon(
                    locations=corners,
                    color=None,
                    fill=True,
    ...
    # Add network links
    '''
    for init_node, term_node in network_links:
        if (init_node not in excluded_nodes and 
            term_node not in excluded_nodes and 
            init_node in node_coords and 
            term_node in node_coords):
            avg_time = (travel_times.get(init_node, min_time) + travel_times.get(term_node, min_time)) / 2
            folium.PolyLine(
                locations=[node_coords[init_node], node_coords[term_node]],
                color=get_color(avg_time, min_time, max_time),
                weight=2,
                opacity=0.8
            ).add_to(mymap)
    '''
    ...
    <div style="position: fixed; 
        ...
        <span style="float:left;margin-top:5px;">{:.1f} min</span>
        <span style="float:right;margin-top:5px;">{:.1f} min</span>
        <br clear="all">
    </div>
...
print("Loading data...")
travel_times_data = parse_travel_times(travel_times_file)
...
# Store locations (keep your complete list here)
store_locations = [
...
# Nodes to exclude
#excluded_nodes = {67, 24, 25, 26, 72, 213, 214} #Origin 1
excluded_nodes = {}
...
output_folder = '/Users/mobjoon/Downloads/accessibility_maps_High_TNCs_prices'
os.makedirs(output_folder, exist_ok=True)  # Ensure the folder exists

# Run the function for multiple origins
for origin in range(1, 3061, 5):
    if origin not in travel_times_data:
        print(f"Origin {origin} not in dataset; skipping.")
        continue
    
    print(f"Creating accessibility map for origin {origin}...")
    mymap = create_combined_accessibility_map(
        travel_times_data,
        node_coords,
        network_links,
        origin,
    ...
    if mymap is not None:
        output_file = os.path.join(output_folder, f"accessibility_map_origin_{origin}.html")
